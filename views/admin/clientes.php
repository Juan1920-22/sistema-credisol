...
$sql = "SELECT u.*,
        (SELECT COUNT(*) FROM solicitudes WHERE cliente_id=u.id) AS total_sol,
        (SELECT COUNT(*) FROM solicitudes WHERE cliente_id=u.id AND estado='desembolsada') AS prestamos_activos,
        (SELECT COALESCE(SUM(saldo_pendiente),0) FROM cartera_prestamos WHERE cliente_id=u.id AND estado='vigente') AS deuda,
        (SELECT saldo FROM cuentas_ahorro WHERE cliente_id=u.id AND estado='activa' LIMIT 1) AS ahorro
        FROM usuarios u WHERE u.rol_id=1";
if ($buscar) $sql .= " AND (u.nombres LIKE '%$buscar%' OR u.apellidos LIKE '%$buscar%' OR u.dni LIKE '%$buscar%' OR u.correo LIKE '%$buscar%')";
...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .b-act{background:#d1fae5;color:#065f46;}.b-bloq{background:#fee2e2;color:#991b1b;}
    ...
    <div class="card">
        <form method="GET">
            <div class="buscar-bar">
                <input type="text" name="buscar" class="buscar" placeholder="Buscar por nombre, DNI o correo..." value="<?= $buscar ?>">
        ...
        <table>
            <thead>
                <tr><th>Cliente</th><th>Correo</th><th>Solicitudes</th><th>Deuda</th><th>Ahorros</th><th>Registro</th><th>Estado</th></tr>
            </thead>
            <tbody>
            ...
            <tr>
                ...
                <td style="text-align:center;font-weight:600;"><?= $c['total_sol'] ?></td>
                <td style="font-weight:600;color:<?= $c['deuda']>0?'#dc2626':'#059669' ?>"><?= soles($c['deuda']??0) ?></td>
                <td style="font-weight:600;color:#059669;"><?= $c['ahorro']!==null ? soles($c['ahorro']) : '—' ?></td>
                <td style="color:#64748b;"><?= fechaCorta($c['creado_en']) ?></td>
                <td><span class="badge <?= $c['activo']?'b-act':'b-bloq' ?>"><?= $c['activo']?'Activo':'Bloqueado' ?></span></td>
            </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
        <div style="margin-top:12px;font-size:.78rem;color:#94a3b8;"><?= count($clientes) ?> clientes encontrados</div>
        <?php endif; ?>
    </div>
</main>
<script>
